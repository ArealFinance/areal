...
# ----------------------------------------------------------------------------
# Step 10 — SDK localnet-mint sync check
# ----------------------------------------------------------------------------
#
# After a successful run_smoke, the local data/e2e-bootstrap.json reflects
# the VPS chain state — including the runtime RWT/USDC test-mint pubkeys
# generated by bootstrap-init.ts (random Keypair.generate() on the VPS).
# Frontend (app.areal.finance) filters its pool catalogue using
# `sdk/src/network/constants.ts::USDC_MINTS.localnet` and
# `RWT_MINTS.localnet` — when those constants drift from the VPS chain
# state, the master pool is invisible to the UI ("Real World Token
# doesn't open"). The backend indexer has the same dependency.
#
# This step compares the local artifact's mints against the values the
# SDK currently exports and prints a clear warning when they diverge.
# The sync itself is operator-driven via scripts/sync-sdk-localnet-mints.sh
# because pinning is a state-bearing decision that affects every
# downstream consumer (SDK version bump → app + backend rebuild +
# redeploy required).
check_sdk_mint_sync() {
  log "step 10: checking SDK localnet mints vs VPS bootstrap artifact"

  if (( DRY_RUN )); then
    log "  (dry-run: skipping sync check)"
    return 0
  fi

  if [[ ! -f "$LOCAL_ARTIFACT_FILE" ]]; then
    log "  no local artifact at $LOCAL_ARTIFACT_FILE; skipping sync check"
    return 0
  fi

  local vps_rwt vps_usdc
  vps_rwt="$(python3 -c "import json; d=json.load(open('$LOCAL_ARTIFACT_FILE')); print(d.get('mints', {}).get('rwt_mint', ''))" 2>/dev/null || echo "")"
  vps_usdc="$(python3 -c "import json; d=json.load(open('$LOCAL_ARTIFACT_FILE')); print(d.get('mints', {}).get('usdc_test_mint', ''))" 2>/dev/null || echo "")"

  if [[ -z "$vps_rwt" || -z "$vps_usdc" ]]; then
    log "  artifact missing mints.rwt_mint or mints.usdc_test_mint; skipping sync check"
    return 0
  fi

  local sdk_constants="$ROOT_DIR/sdk/src/network/constants.ts"
  if [[ ! -f "$sdk_constants" ]]; then
    log "  SDK constants file missing at $sdk_constants; skipping sync check"
    return 0
  fi

  # Match the `localnet: new PublicKey('<base58>')` line in each *_MINTS
  # block. Falls back to empty on parse failure (sync check then warns,
  # which is itself useful signal).
  local sdk_usdc sdk_rwt
...
PY
  ...
  if [[ "$sdk_usdc" == "$vps_usdc" && "$sdk_rwt" == "$vps_rwt" ]]; then
    log "  SDK localnet mints match VPS chain state — OK"
    log "    SDK/VPS USDC: $vps_usdc"
    log "    SDK/VPS RWT:  $vps_rwt"
    return 0
  fi

  log "WARNING: SDK localnet mints are out of sync with VPS state."
  log "  SDK USDC:  $sdk_usdc"
  log "  VPS USDC:  $vps_usdc"
  log "  SDK RWT:   $sdk_rwt"
  log "  VPS RWT:   $vps_rwt"
  log "  Run scripts/sync-sdk-localnet-mints.sh to update + bump SDK."
  log "  App + backend rebuild required after sync."
}
...
main() {
  ...
  log "Fornex deploy complete — all 10 steps green"
}
